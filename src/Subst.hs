...
varChanger local bound t = go 0 t where
  ...
  go n (Lam p y ty (Bound1 t))   = Lam p y ty (Bound1 (go (n+1) t))
  go n (App p l r)   = App p (go n l) (go n r)
  go n (Fix p f fty x xty (Bound2 t)) = Fix p f fty x xty (Bound2 (go (n+2) t))
  ...
  go n (Let p v vty m (Bound1 o)) = Let p v vty (go n m) (Bound1 (go (n+1) o))
...
open :: Name -> Scope One info Var -> Tm info Var
open nm (Bound1 t) = varChanger (\_ p n -> V p (Free n)) bnd t
...
open2 :: Name -> Name -> Scope Two info Var -> Tm info Var
open2 nm1 nm2 (Bound2 t) = varChanger (\_ p n -> V p (Free n)) bnd t
   where bnd depth p i | i <  depth   = V p (Bound i)
...
-- nombres frescos.
subst :: Tm info Var -> Scope One info Var -> Tm info Var
subst n (Bound1 m) = varChanger (\_ p n -> V p (Free n)) bnd m
   where bnd depth p i 
             | i <  depth = V p (Bound i)
             | i == depth = n
...
subst2 :: Tm info Var -> Tm info Var -> Scope Two info Var -> Tm info Var
subst2 n1 n2 (Bound2 m) = varChanger (\_ p n -> V p (Free n)) bnd m
   where bnd depth p i 
             | i <  depth = V p (Bound i)
             | i == depth = n2
             | i == depth+1 = n1
...
close :: Name -> Tm info Var -> Scope One info Var 
close nm t = Bound1 (varChanger lcl (\_ p i -> V p (Bound i)) t)
 where lcl depth p y =
            if y==nm then V p (Bound depth)
                     else V p (Free y)

-- Similar a `close` pero para el caso de cerrar dos nombres.
close2 :: Name -> Name -> Tm info Var -> Scope Two info Var
close2 nm1 nm2 t = Bound2 (varChanger lcl (\_ p i -> V p (Bound i)) t)
